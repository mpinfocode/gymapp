...
Lingua: **solo italiano** (UI e istruzioni esercizi; nessuna scelta lingua, `en` resta nel JSON solo come fallback tecnico). Tutto gratis, nessun account, nessun backend nella v1.
...
6. **Impostazioni** — nome, unità, recupero di default, haptics, download offline dei media (progress), esporta/importa backup (ShareLink / fileImporter), crediti e licenze.
