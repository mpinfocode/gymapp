...
**Scheda con l'AI (richiesta dell'utente, 2026-09-18)**: nella tab Scheda il "+" per una nuova scheda apre la scelta **Manuale** (flusso esistente) oppure **Con l'AI**: wizard a domande CHIUSE, sempre lo stesso, una per schermata (obiettivo; giorni a settimana 2-6; divisione consigliata/full body/upper-lower/push-pull-legs/gruppi muscolari, filtrata dai giorni; esperienza; tempo per seduta 45/60/75-90; attrezzatura palestra/manubri+panca/corpo libero; zone su cui insistere, max 2, facoltativa; zone da proteggere, facoltativa, con avviso "non è un parere medico"; cardio; durata 4/6/8 settimane) → riepilogo → generazione → anteprima (giorni, esercizi, serie × ripetizioni, recuperi, barra dei muscoli colpiti) → "Usa questa scheda" / "Rigenera" / "Annulla". Nessuna chat, nessun testo libero. L'AI NON legge il dataset: il telefono seleziona i candidati da una selezione curata (~250 esercizi etichettati, `GymCore/Generator`) in base alle risposte e li invia in forma compatta; il modello risponde SOLO con JSON di id, validato e riparato sul telefono; se non valido un secondo tentativo, poi errore chiaro. **Chiave OpenRouter inserita dall'utente in Impostazioni e salvata nel Portachiavi** (mai nel repo, mai nella build, mai su un server, mai mostrata in chiaro dopo il salvataggio); modello configurabile in Impostazioni con un default economico. Senza chiave o senza rete: generatore deterministico di riserva senza AI. I carichi non vengono generati. Nessun backend (niente Vercel/Neon).
...
3. Secondari: dopo la pulizia in `ExerciseCorrections` e la classificazione sinergista/stabilizzatore (`SecondaryMuscles.swift`) ENTRANO nella distribuzione dei muscoli colpiti della scheda con peso ridotto (principale 1, sinergista 0,5, stabilizzatore 0,25), su richiesta dell'utente.
