...
public struct CuratedExercise: Sendable, Hashable, Identifiable {
    ...
    /// Zone che l'esercizio carica in modo **netto**: con quella zona da
    /// proteggere l'esercizio non entra nemmeno fra i candidati.
    ///
    /// Sono i movimenti che un preparatore toglie senza discutere: panca e lento
    /// con il bilanciere e dip per una spalla, stacco e good morning per una
    /// schiena bassa, sissy squat e affondi saltati per un ginocchio, skull
    /// crusher e curl al bilanciere dritto per un gomito.
    public let avoid: Set<StressZone>
    /// Zone che l'esercizio carica **in parte**: si può usare, ma uno per seduta
    /// e mai come primo esercizio.
    ///
    /// Sono le scelte che con un po' di attenzione restano lecite: lo stacco
    /// rumeno con i manubri, la leg extension, lo squat al multipower, le
    /// estensioni per i tricipiti con i manubri.
    public let caution: Set<StressZone>

    /// Inizializzatore posizionale: la tabella qui sotto è lunga, e con le
    /// etichette diventerebbe illeggibile. L'ordine è
    /// id, pattern, tipo, livello, attrezzatura, priorità; le due liste di zone
    /// restano con l'etichetta perché confonderle sarebbe un errore grave.
    public init(
        _ id: String,
        ...
        _ priority: Int,
        avoid: Set<StressZone> = [],
        caution: Set<StressZone> = []
    ) {
        self.id = id
        ...
        self.avoid = avoid
        // Una zona non può essere insieme da evitare e da usare con cautela:
        // vince il divieto, così una svista nella tabella non allenta il filtro.
        self.caution = caution.subtracting(avoid)
    }

    /// Tutte le zone toccate, a qualunque livello.
    public var stress: Set<StressZone> { avoid.union(caution) }
    ...
    /// `true` se l'esercizio non è vietato da nessuna delle zone da proteggere.
    public func respects(protectedZones: Set<StressZone>) -> Bool { avoid.isDisjoint(with: protectedZones) }

    /// `true` se l'esercizio è ammesso ma va trattato con cautela.
    public func needsCare(protectedZones: Set<StressZone>) -> Bool { !caution.isDisjoint(with: protectedZones) }
}
...
public struct ResolvedExercise: Sendable, Hashable, Identifiable {
    ...
    public var avoidZones: Set<StressZone> { curated.avoid }
    public var cautionZones: Set<StressZone> { curated.caution }
...
public enum CuratedExercisePool {
    ...
    public static let all: [CuratedExercise] = [
        ...
        // Sono il lavoro diretto per la zona lombare: con la schiena bassa da
        // proteggere non si vietano, si usano leggere e mai per prime.
        .init("0489", .backExtension, .isolation, .beginner, .bodyweightBands, 1, caution: [.lowerBack]),  // hyperextension
        .init("0573", .backExtension, .isolation, .beginner, .gym, 1, caution: [.lowerBack]),  // lever back extension
        .init("0593", .backExtension, .isolation, .beginner, .gym, 3, caution: [.lowerBack]),  // lever reverse hyperextension
        ...
        // Pullover: la spalla va in flessione massima sotto carico.
        .init("2285", .backIsolation, .isolation, .beginner, .gym, 3, avoid: [.shoulders]),  // lever pullover
        ...
        .init("0439", .bicepsCurl, .isolation, .intermediate, .dumbbellBench, 3, caution: [.wristsElbows]),  // dumbbell zottman curl
        // Bilanciere dritto: il polso resta bloccato in supinazione, è il curl
        // che dà fastidio ai gomiti. L'ez e i manubri fanno lo stesso lavoro.
        .init("0031", .bicepsCurl, .isolation, .beginner, .gym, 1, avoid: [.wristsElbows]),  // barbell curl
        ...
        .init("0070", .bicepsCurl, .isolation, .beginner, .gym, 2, avoid: [.wristsElbows]),  // barbell preacher curl
        ...
        // Bilanciere sulle spalle: carico assiale vero, non è roba da prima scheda.
        .init("1372", .calf, .isolation, .intermediate, .gym, 2, caution: [.lowerBack]),  // barbell standing calf raise
        .init("2289", .calf, .isolation, .beginner, .gym, 2),  // lever calf press
        .init("0088", .calf, .isolation, .intermediate, .gym, 3),  // barbell seated calf raise
        ...
        .init("1160", .cardio, .isolation, .intermediate, .bodyweightBands, 1, avoid: [.knees]),  // burpee
        .init("2612", .cardio, .isolation, .beginner, .bodyweightBands, 1, caution: [.knees]),  // jump rope
        .init("3636", .cardio, .isolation, .beginner, .bodyweightBands, 2),  // high knee against wall
        .init("3361", .cardio, .isolation, .intermediate, .bodyweightBands, 3, avoid: [.knees]),  // skater hops
        ...
        // Croci con i manubri: la spalla resta aperta sotto carico in allungamento.
        // Con cautela si possono fare corte e leggere, il pullover no.
        .init("0308", .chestIsolation, .isolation, .intermediate, .dumbbellBench, 2, caution: [.shoulders]),  // dumbbell fly
        .init("0319", .chestIsolation, .isolation, .intermediate, .dumbbellBench, 2, caution: [.shoulders]),  // dumbbell incline fly
        .init("0375", .chestIsolation, .isolation, .intermediate, .dumbbellBench, 3, avoid: [.shoulders]),  // dumbbell pullover
        ...
        .init("0805", .coreAntiExtension, .isolation, .advanced, .bodyweightBands, 3, avoid: [.lowerBack]),  // suspended abdominal fallout
        .init("0857", .coreAntiExtension, .isolation, .advanced, .dumbbellBench, 3, avoid: [.lowerBack], caution: [.shoulders]),  // wheel rollerout
        .init("0084", .coreAntiExtension, .isolation, .advanced, .gym, 3, avoid: [.lowerBack], caution: [.shoulders]),  // barbell rollerout

        // MARK: Core in flessione
        .init("0274", .coreFlexion, .isolation, .beginner, .bodyweightBands, 1),  // crunch floor
        .init("0472", .coreFlexion, .isolation, .intermediate, .bodyweightBands, 1, caution: [.shoulders]),  // hanging leg raise
        ...
        .init("0475", .coreFlexion, .isolation, .advanced, .bodyweightBands, 2, caution: [.shoulders]),  // hanging straight leg raise
        ...
        .init("0687", .coreRotation, .isolation, .beginner, .bodyweightBands, 2, caution: [.lowerBack]),  // russian twist
        .init("1015", .coreRotation, .isolation, .beginner, .bodyweightBands, 3),  // band vertical pallof press
        .init("0407", .coreRotation, .isolation, .beginner, .dumbbellBench, 2, caution: [.lowerBack]),  // dumbbell side bend
        .init("0243", .coreRotation, .isolation, .beginner, .gym, 2),  // cable twist
        .init("0222", .coreRotation, .isolation, .beginner, .gym, 3, caution: [.lowerBack]),  // cable side bend

        // MARK: Avambracci
        // Sono il lavoro diretto per polsi e gomiti: con quella zona da
        // proteggere restano leggeri e mai per primi, non spariscono.
        .init("1016", .forearm, .isolation, .beginner, .bodyweightBands, 1, caution: [.wristsElbows]),  // band wrist curl
        .init("0994", .forearm, .isolation, .beginner, .bodyweightBands, 3, caution: [.wristsElbows]),  // band reverse wrist curl
        .init("0369", .forearm, .isolation, .beginner, .dumbbellBench, 1, caution: [.wristsElbows]),  // dumbbell over bench wrist curl
        .init("0385", .forearm, .isolation, .beginner, .dumbbellBench, 2, caution: [.wristsElbows]),  // dumbbell reverse wrist curl
        .init("0126", .forearm, .isolation, .beginner, .gym, 1, caution: [.wristsElbows]),  // barbell wrist curl
        .init("0082", .forearm, .isolation, .beginner, .gym, 3, caution: [.wristsElbows]),  // barbell reverse wrist curl
        .init("0210", .forearm, .isolation, .beginner, .gym, 3, caution: [.wristsElbows]),  // cable reverse wrist curl
        .init("0247", .forearm, .isolation, .beginner, .gym, 3, caution: [.wristsElbows]),  // cable wrist curl

        // MARK: Anca dominante
        // Stacchi da terra, good morning e stacchi a gambe tese con il bilanciere:
        // il momento flettente sulla lombare è il punto dell'esercizio. Con la
        // schiena bassa da proteggere restano ponte per i glutei, hip thrust,
        // pull through e, con cautela, le versioni con i manubri.
        .init("1009", .hinge, .compound, .beginner, .bodyweightBands, 1, caution: [.lowerBack]),  // band stiff leg deadlift
        ...
        .init("0991", .hinge, .compound, .beginner, .bodyweightBands, 2, caution: [.lowerBack]),  // band pull through
        ...
        .init("0300", .hinge, .compound, .beginner, .dumbbellBench, 1, caution: [.lowerBack]),  // dumbbell deadlift
        .init("1459", .hinge, .compound, .beginner, .dumbbellBench, 1, caution: [.lowerBack]),  // dumbbell romanian deadlift
        .init("0432", .hinge, .compound, .intermediate, .dumbbellBench, 2, avoid: [.lowerBack]),  // dumbbell stiff leg deadlift
        .init("0549", .hinge, .compound, .intermediate, .dumbbellBench, 2, avoid: [.lowerBack]),  // kettlebell swing
        .init("1757", .hinge, .compound, .intermediate, .dumbbellBench, 3, caution: [.lowerBack]),  // dumbbell single leg deadlift
        .init("0032", .hinge, .compound, .advanced, .gym, 1, avoid: [.lowerBack]),  // barbell deadlift
        .init("0085", .hinge, .compound, .intermediate, .gym, 1, avoid: [.lowerBack]),  // barbell romanian deadlift
        .init("0116", .hinge, .compound, .intermediate, .gym, 2, avoid: [.lowerBack]),  // barbell straight leg deadlift
        .init("0117", .hinge, .compound, .advanced, .gym, 2, avoid: [.lowerBack]),  // barbell sumo deadlift
        .init("0196", .hinge, .compound, .beginner, .gym, 2, caution: [.lowerBack]),  // cable pull through (with rope)
        .init("1409", .hinge, .compound, .intermediate, .gym, 2),  // barbell glute bridge
        .init("0044", .hinge, .compound, .advanced, .gym, 3, avoid: [.lowerBack]),  // barbell good morning
        .init("0811", .hinge, .compound, .intermediate, .gym, 3, avoid: [.lowerBack]),  // trap bar deadlift

        // MARK: Tirata orizzontale
        // Il rematore con il busto flesso tiene la lombare in isometria per tutta
        // la serie: con la schiena bassa da proteggere si usano le versioni
        // appoggiate (panca, macchina, cavo seduto).
        ...
        .init("0293", .horizontalPull, .compound, .beginner, .dumbbellBench, 1, avoid: [.lowerBack]),  // dumbbell bent over row
        .init("0027", .horizontalPull, .compound, .intermediate, .gym, 1, avoid: [.lowerBack]),  // barbell bent over row
        ...
        .init("0606", .horizontalPull, .compound, .intermediate, .gym, 2, avoid: [.lowerBack]),  // lever t bar row
        .init("0118", .horizontalPull, .compound, .intermediate, .gym, 3, avoid: [.lowerBack]),  // barbell reverse grip bent over row
        .init("0218", .horizontalPull, .compound, .beginner, .gym, 3, caution: [.shoulders]),  // cable seated wide-grip row
        .init("1359", .horizontalPull, .compound, .intermediate, .gym, 3, avoid: [.lowerBack]),  // smith bent over row
        .init("3017", .horizontalPull, .compound, .advanced, .gym, 3, avoid: [.lowerBack]),  // barbell pendlay row

        // MARK: Spinta orizzontale
        // Panca piana, inclinata e declinata con il bilanciere: la barra obbliga
        // la spalla a un percorso fisso in extrarotazione e abduzione. Con la
        // spalla da proteggere restano macchine, manubri e piegamenti; i dip
        // sono fuori per tutti e due i motivi (spalla e gomito).
        .init("0251", .horizontalPush, .compound, .intermediate, .bodyweightBands, 1, avoid: [.shoulders, .wristsElbows]),  // chest dip
        .init("0259", .horizontalPush, .compound, .beginner, .bodyweightBands, 1, caution: [.wristsElbows]),  // close-grip push-up
        ...
        .init("0279", .horizontalPush, .compound, .intermediate, .bodyweightBands, 2, caution: [.shoulders]),  // decline push-up
        .init("0283", .horizontalPush, .compound, .intermediate, .bodyweightBands, 2, avoid: [.wristsElbows]),  // diamond push-up
        .init("1254", .horizontalPush, .compound, .beginner, .bodyweightBands, 2),  // band bench press
        .init("1311", .horizontalPush, .compound, .beginner, .bodyweightBands, 3, caution: [.shoulders]),  // wide hand push up
        ...
        .init("0301", .horizontalPush, .compound, .intermediate, .dumbbellBench, 3, caution: [.shoulders]),  // dumbbell decline bench press
        .init("0025", .horizontalPush, .compound, .intermediate, .gym, 1, avoid: [.shoulders]),  // barbell bench press
        .init("0030", .horizontalPush, .compound, .intermediate, .gym, 1, avoid: [.wristsElbows], caution: [.shoulders]),  // barbell close-grip bench press
        .init("0047", .horizontalPush, .compound, .intermediate, .gym, 1, avoid: [.shoulders]),  // barbell incline bench press
        ...
        .init("0033", .horizontalPush, .compound, .intermediate, .gym, 2, avoid: [.shoulders]),  // barbell decline bench press
        .init("0748", .horizontalPush, .compound, .beginner, .gym, 2, caution: [.shoulders]),  // smith bench press
        .init("0757", .horizontalPush, .compound, .beginner, .gym, 2, caution: [.shoulders]),  // smith incline bench press
        .init("2144", .horizontalPush, .compound, .beginner, .gym, 2),  // cable seated chest press
        .init("0751", .horizontalPush, .compound, .beginner, .gym, 3, avoid: [.wristsElbows]),  // smith close-grip bench press
        ...
        // Tirata al mento: intrarotazione sotto carico, il classico conflitto.
        .init("0246", .lateralRaise, .isolation, .intermediate, .gym, 3, avoid: [.shoulders, .wristsElbows]),  // cable upright row
        ...
        .init("3193", .legIsolation, .compound, .advanced, .bodyweightBands, 2, caution: [.knees]),  // glute-ham raise
        ...
        // Sissy squat: ginocchio in flessione massima oltre la punta, sotto carico.
        .init("1489", .legIsolation, .isolation, .advanced, .bodyweightBands, 3, avoid: [.knees]),  // sissy squat
        ...
        // Leg extension: pesante è il classico fastidio rotuleo, leggera è una
        // delle poche cose che si possono fare con un ginocchio delicato.
        .init("0585", .legIsolation, .isolation, .beginner, .gym, 1, caution: [.knees]),  // lever leg extension
        ...
        .init("1460", .lunge, .compound, .beginner, .bodyweightBands, 1, caution: [.knees]),  // walking lunge
        ...
        .init("0336", .lunge, .compound, .beginner, .dumbbellBench, 1, caution: [.knees]),  // dumbbell lunge
        ...
        .init("0431", .lunge, .compound, .beginner, .dumbbellBench, 2, caution: [.knees]),  // dumbbell step-up
        .init("0054", .lunge, .compound, .intermediate, .gym, 2, avoid: [.knees], caution: [.lowerBack]),  // barbell lunge
        .init("0078", .lunge, .compound, .intermediate, .gym, 3, avoid: [.knees], caution: [.lowerBack]),  // barbell rear lunge
        .init("0114", .lunge, .compound, .intermediate, .gym, 3, avoid: [.knees], caution: [.lowerBack]),  // barbell step-up
        .init("0768", .lunge, .compound, .intermediate, .gym, 3, avoid: [.knees]),  // smith single leg split squat
        ...
        .init("0095", .shrug, .isolation, .intermediate, .gym, 1, caution: [.lowerBack]),  // barbell shrug
        .init("0220", .shrug, .isolation, .beginner, .gym, 3),  // cable shrug

        // MARK: Squat e ginocchio dominante
        // Squat con il bilanciere: carico assiale sulla colonna e ginocchio in
        // flessione profonda. Non è un esercizio da prima scheda e sparisce con
        // schiena bassa o ginocchia da proteggere; pressa e multipower restano.
        ...
        .init("0514", .squat, .compound, .advanced, .bodyweightBands, 3, avoid: [.knees]),  // jump squat
        ...
        .init("0043", .squat, .compound, .intermediate, .gym, 1, avoid: [.knees, .lowerBack]),  // barbell full squat
        .init("0739", .squat, .compound, .beginner, .gym, 1),  // sled 45° leg press
        .init("0042", .squat, .compound, .advanced, .gym, 2, avoid: [.knees, .lowerBack], caution: [.wristsElbows]),  // barbell front squat
        .init("0743", .squat, .compound, .beginner, .gym, 2, caution: [.knees]),  // sled hack squat
        .init("0770", .squat, .compound, .beginner, .gym, 2, caution: [.knees]),  // smith squat
        .init("1436", .squat, .compound, .intermediate, .gym, 2, avoid: [.knees, .lowerBack]),  // barbell high bar squat
        .init("0750", .squat, .compound, .beginner, .gym, 3, caution: [.knees]),  // smith chair squat

        // MARK: Tricipiti
        // I dip caricano la spalla in estensione profonda e il gomito in chiusura:
        // con una di quelle due zone da proteggere non si usano, punto.
        .init("0814", .tricepsExtension, .compound, .intermediate, .bodyweightBands, 1, avoid: [.shoulders, .wristsElbows]),  // triceps dip
        .init("0998", .tricepsExtension, .isolation, .beginner, .bodyweightBands, 1),  // band side triceps extension
        .init("0129", .tricepsExtension, .compound, .beginner, .bodyweightBands, 2, avoid: [.shoulders, .wristsElbows]),  // bench dip (knees bent)
        .init("2188", .tricepsExtension, .isolation, .beginner, .dumbbellBench, 1, caution: [.wristsElbows, .shoulders]),  // dumbbell seated triceps extension
        .init("0333", .tricepsExtension, .isolation, .beginner, .dumbbellBench, 2),  // dumbbell kickback
        .init("0351", .tricepsExtension, .isolation, .beginner, .dumbbellBench, 2, caution: [.wristsElbows]),  // dumbbell lying triceps extension
        .init("0430", .tricepsExtension, .isolation, .beginner, .dumbbellBench, 2, caution: [.wristsElbows]),  // dumbbell standing triceps extension
        ...
        // Skull crusher e french press: bilanciere dritto, gomito in appoggio.
        .init("0060", .tricepsExtension, .isolation, .intermediate, .gym, 2, avoid: [.wristsElbows]),  // barbell lying triceps extension skull crusher
        .init("0241", .tricepsExtension, .isolation, .beginner, .gym, 2),  // cable triceps pushdown (v-bar)
        .init("0453", .tricepsExtension, .isolation, .intermediate, .gym, 2, avoid: [.wristsElbows], caution: [.shoulders]),  // ez barbell seated triceps extension
        .init("1451", .tricepsExtension, .isolation, .beginner, .gym, 2, avoid: [.shoulders, .wristsElbows]),  // lever seated dip
        .init("0092", .tricepsExtension, .isolation, .intermediate, .gym, 3, avoid: [.wristsElbows], caution: [.shoulders]),  // barbell seated overhead triceps extension
        ...
        .init("1749", .tricepsExtension, .isolation, .intermediate, .gym, 3, avoid: [.wristsElbows], caution: [.shoulders]),  // ez bar standing french press

        // MARK: Tirata verticale
        // Presa prona larga: la spalla lavora in abduzione ed extrarotazione.
        // Presa neutra e supina restano pulite, la lat machine davanti pure.
        .init("0652", .verticalPull, .compound, .advanced, .bodyweightBands, 1, caution: [.shoulders, .wristsElbows]),  // pull-up
        .init("1013", .verticalPull, .compound, .beginner, .bodyweightBands, 1),  // band underhand pulldown
        .init("1326", .verticalPull, .compound, .advanced, .bodyweightBands, 1, caution: [.wristsElbows]),  // chin-up
        .init("0140", .verticalPull, .compound, .advanced, .bodyweightBands, 2, caution: [.wristsElbows]),  // biceps pull-up
        ...
        // Lento avanti e dietro con il bilanciere: la barra obbliga a portare la
        // testa fuori dalla traiettoria e chiude lo spazio subacromiale. Restano
        // macchina, cavi, elastico e manubri a presa neutra.
        ...
        .init("0426", .verticalPush, .compound, .intermediate, .dumbbellBench, 1, caution: [.shoulders]),  // dumbbell standing overhead press
        .init("2137", .verticalPush, .compound, .intermediate, .dumbbellBench, 2, caution: [.shoulders]),  // dumbbell arnold press
        .init("0553", .verticalPush, .compound, .intermediate, .dumbbellBench, 3, caution: [.shoulders]),  // kettlebell two arm military press
        .init("0091", .verticalPush, .compound, .intermediate, .gym, 1, avoid: [.shoulders]),  // barbell seated overhead press
        .init("0603", .verticalPush, .compound, .beginner, .gym, 1),  // lever shoulder press
        .init("1456", .verticalPush, .compound, .intermediate, .gym, 2, avoid: [.shoulders], caution: [.wristsElbows]),  // barbell standing close grip military press
        .init("0086", .verticalPush, .compound, .advanced, .gym, 3, avoid: [.shoulders]),  // barbell seated behind head military press
        .init("0219", .verticalPush, .compound, .beginner, .gym, 3),  // cable shoulder press
        .init("0766", .verticalPush, .compound, .beginner, .gym, 3, caution: [.shoulders]),  // smith shoulder press
    ]
