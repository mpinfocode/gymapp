...
public struct GeneratorCandidate: Sendable, Hashable, Identifiable {
    ...
    public var compactLine: String {
        "\(id)|\(shortName)|\(group.displayName)|\(equipment)|\(pattern.displayName)|\(kind.compactCode)"
    }
}
...
public struct GeneratorCandidates: Sendable {
    ...
    /// Ordine di preferenza: prima i classici, poi ciò che serve meno
    /// attrezzatura (funziona ovunque), poi il livello più accessibile.
    ...
    public func items(pattern: MovementPattern) -> [GeneratorCandidate] {
        items.filter { $0.pattern == pattern }.sorted(by: GeneratorCandidates.isBetter)
    }

    /// Candidati di quel gruppo muscolare, nell'ordine di preferenza.
    public func items(group: MuscleGroup) -> [GeneratorCandidate] {
        items.filter { $0.group == group }.sorted(by: GeneratorCandidates.isBetter)
    }
    ...
    /// L'elenco compatto, una riga per esercizio.
    public var compactList: String {
        items.map(\.compactLine).joined(separator: "\n")
    }
